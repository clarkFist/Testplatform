...
public:
	...
	void SetLimitText(UINT nMax) { m_nMaxChars = nMax; }
#if _MSC_VER >= 1600
	void SetCharFilterFn(const std::function<BOOL(UINT nChar)>& pfn)
	{
		m_pfnCharFilter = pfn;
	}
#else
	void SetCharFilterFn(BOOL (*pfn)(UINT nChar))
	{
		m_pfnCharFilter = pfn;
	}
#endif
...
private:
	UINT	   m_nMaxChars;
#if _MSC_VER >= 1600
	std::function<BOOL(UINT nChar)> m_pfnCharFilter;
#else
	BOOL	(*m_pfnCharFilter)(UINT nChar);
#endif
