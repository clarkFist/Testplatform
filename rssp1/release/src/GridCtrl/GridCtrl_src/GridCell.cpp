...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		if (m_nMaxChars > 0)
			((CInPlaceEdit*)m_pEditWnd)->SetLimitText(m_nMaxChars);
		((CInPlaceEdit*)m_pEditWnd)->SetCharFilterFn(m_pfnCharFilter);
    }
    return TRUE;
}
...
#if(WINVER >= 0x0600) // vista, win 7, or higher
	ncm.cbSize = sizeof(NONCLIENTMETRICS) - sizeof(ncm.iPaddedBorderWidth); // dys£¬http://hi.baidu.com/guid/blog/item/5d9e019535169f007af4806a.html
#else
