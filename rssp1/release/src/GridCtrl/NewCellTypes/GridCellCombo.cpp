...
void CInPlaceList::OnSelChange()
{
	if ((GetStyle() & CBS_DROPDOWNLIST) == CBS_DROPDOWNLIST)
	{
		...
		if (::IsWindow(m_hWnd))
			GetWindowText(str);
		if (str == "ÎüÆð")
			((CGridCtrl *)GetParent())->SendMessageToParent(m_nRow, m_nCol, GVN_SELCHANGEDUP);
		if (str == "ÂäÏÂ")
			((CGridCtrl *)GetParent())->SendMessageToParent(m_nRow, m_nCol, GVN_SELCHANGEDDOWN);
		EndEdit();
	}
}
