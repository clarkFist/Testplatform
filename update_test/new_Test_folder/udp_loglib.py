...
class myGevent(greenlet.Greenlet):
    ...
    def empty():
        ...
        def buzy(count, ttime):
            Public.mPrint('cpu buzy', count, ttime)
        ...
        while Public.thread_flag.setdefault('empty', True):
            ...
            if cur_time - temp >= 1:
                if count < 5 or cur_time - temp > 1.2:
                    buzy(count, cur_time - temp)
                temp = cur_time
                count = 0


DEBUG_MODE = Public.isDebugMode()  # 通过
if DEBUG_MODE:
    threadpool.sys.settrace = Public.none_func
...
class Base_ui(object):
    ...
    def show_msg(self, event, name, val):
        self.return_val[name] = val.get()

    def adaptor(self, fun, **kwds):
        return lambda event, fun=fun, kwds=kwds: fun(event, **kwds)

    def combo_add(self, name, values, state='readonly'):
        self.column = 1
        tk.Label(self.win, text=name).grid(row=self.row, column=self.column, sticky='W')
        self.column = 2
        box = ttk.Combobox(self.win, state=state)
        box.grid(row=self.row, column=self.column)
        self.row += 1
        box["values"] = values
        box.bind("<<ComboboxSelected>>",
                 self.adaptor(self.show_msg, name=name, val=box))
        box.current(0)
        self.return_val[name] = box.get()

    def path_add(self, name, values):
        self.column = 1
        tk.Label(self.win, text=name).grid(row=self.row, column=self.column, sticky='W')
        self.column = 2
        box = ttk.Combobox(self.win)
        box.grid(row=self.row, column=self.column)
        self.row += 1
        box["values"] = tuple(values)
        #box.bind("<Button-1>", input_ui.adaptor(self.path_select,name = name,val = box))
        box.bind("<<ComboboxSelected>>",
                 self.adaptor(self.path_select, name=name, val=box))
        box.current(0)
        self.return_val[name] = box.get()
    ...
    def path_select(self, event, name, val):
        path = val.get()
        if path == 'select':
            val.set(filedialog.askdirectory())
        if len(val.get()) > 0:
            self.show_msg(event, name, val)
        else:
            val.set(path)
...
class input_ui(Base_ui):
    ...
    def __init__(self):
        ...
        ui = self
        self.selet_list = ['./Log_Log/default', 'select']

        ui.win.after(10, self.OnAfter)
        ui.mainloop()
    ...
    def OnAfter(self):
        ...
        if len(temp_path) > 0:
            self.selet_list.insert(0, temp_path)
        ui.path_add('path', self.selet_list)
        button = ttk.Button(ui.win, text='确定', command=ui.close)
        button.grid(row=ui.row, column=1)
        ui.win.bind("<Key-Return>", ui.close)

    def value_get(self):
        return super(input_ui, self).value_get()['path'] + '/'
...
class UdpRecvFrame:
    ...
    def transform(self, dst_2):
        ''''''
        _parse = self._parse
        struct = UdpRecvFrame.udpFrame_struct  #先解析头文件
        dst = self
        frame = self.frame
        _parse(struct, dst, frame)
        ...
        struct = self.__mtype._struct
        _parse(struct, self.__mtype, frame)
...
class VCU_VIV:
    ...
    def __get_function(self, retry, cpu, function, *args, **kwargs):
        ...
        while retry >= 0:
            ...
            except (socket.timeout, TftpException) as e:
                e
                retry -= 1
                gevent.sleep(1)
                continue
    ...
    @funclog
    def get_functionmode(self, retry = 3):
...
#         for x in dir(self):
#             if re.search('^__.+__$', x) is None:
#                 if x in dst.keys():
#                     myAssert(dst[x] == getattr(self, x))
#                 else:
#                     dst[x] = getattr(self, x)
...
class Conf(Default):
    ...
    __control = '02'
    ####
    __flag_dict = {'serial': '00', 'udp': '01', 'all': '03', 'tsk': '04'}
    __cycle_dict = {'all': 3} 
    __control_dict = {'Shutdown': 0, 'FaultFifter': 1, 'TriggerFaultFifter': 2, 'ReprogflagOnDump': 3, 'toStandby': 4}
    ###
...
class Log(Default):
    ...
    def __init__(self):
        self.num_dict = {}  #保存不同cpu的消息序号，检查消息丢失
    ...
    def _default_run(self, frame):
        self.__src = frame.srcNodeID
        #cls.__src = int(time.time() * 1000) % 10 + 10
        self.src_lru, self.src_cpu = VCU.node2lru(self.__src)
...
class Reply(Default):
    ...
    def _reply_Recv(cls, string):
        ...
        if time.time() - FrameLib.respond_list[0][1] > 50.0:
            ...
            mPrint('_reply_Recv,%s', time.time() - FrameLib.respond_list[0][1])
...
class TestPlatForm(object):
    ...
    def get_active_mvcu(self) -> int:
        '''获取主mvcu 的槽道号'''
        from trys import Trys
        funcMode_dict = {0: 'MVCUA', 3: 'MVCUA', -1: 'VIV_ERROR'}
        ...
        func_modes = trys(get_funcMode)()
        for module_3, funcMode in func_modes:
            if funcMode_dict[funcMode] == 'MVCUA':
                rt.append(module_3)
    ...
    def tst_set(self, slots: str, frame: UdpSendFrame, openflag=True, autowait = True) -> None:
        ...
        for moudle_3 in moudle_3s:
            ...
            if int(frame.dstNodeID, 16) % 4 == 0: # moudleid 为偶数，则槽道号为基数
                self._client_odd.send(frame)  #发送给CPUA
            else:
                self._client_even.send(frame)
        if autowait:
    ...
    def _log_recv(self, client: UdpClient):
        ...
        while Public.thread_flag['recv']:
            ...
            if data is not None:
                ...
                #默认vcu只会主动往外发log 消息，即非回复消息frame.mtype 始终指向Log
    ...
    def _set_client(self, mode = [], dst_svcu = None, retry = 5):
        ...
        if isinstance(mode, str):
            mode = re.split(r'[,，.]', mode)
        for i in range(len(mode)):
            mode[i] = int(mode[i])
        if retry < 0:
            raise Exception( mode, 'ip is not reachable')
