...
def get_version(module_3):
    ...
    sub_p = {}
    output_dict = {}
    cmd = 'java -jar rut-2.0-SNAPSHOT.jar  %s %s . VCU_CW_CPU_A_ROM_GUEST_NOETH.S19 VCU_GCC_CPU_B_ROM_GUEST_NOETH.S19 5 0'
    for retry in range(3):
        for i in module_3:
            ip = default_ip
            p = subprocess.Popen(cmd % (lruget(i), ip), shell = False, cwd = os.path.abspath(os.path.dirname(__file__)), 
...
def update_check(sub_p, act):
    rt_code = code_dict['running']
    fail_list = []
    for key, p in sub_p.items():
        if p.poll() is not None:
            cur_rename = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'log', 
                                      slot_ip.get(key, default_ip), 'IMAGE_LRU_%s.txt' % lruget(key))
            dst_rename = '%s_IMAGE_LRU_%s.txt'
            with open(cur_rename, 'r') as f:
                log_content = f.read()
            if act == 0:
                pattren = ('Restore REPROG safety flag on CPU A.+Restore REPROG safety flag on CPU B' +
                           '.+Get command status for CPU A.+Get command status for CPU B.+update mirror image on CPU A' + 
                           '.+update mirror image on CPU B.+Success')
            elif act == 4:
                ...
                cur_rename = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'log', 
                                          slot_ip.get(key, default_ip), 'CONFIG_LRU_%s.txt' % lruget(key))
                dst_rename = '%s_CONFIG_LRU_%s.txt'
            ...
            if state is False:
                rt_code = code_dict['fail']
                fail_list.append(key)
                
                dst_name = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'log', 
                                        slot_ip.get(key, default_ip), dst_rename % (gettime(), lruget(key)) )
                os.rename(cur_rename, dst_name)
        else:
            fail_list.append(key)
            
    if len(fail_list) == 0:
        return code_dict['success'], fail_list
    else:
        return rt_code, fail_list
...
def update(fileType: str, act, module_3, dirname = '', block = True, retry = 3):
    '''
    Descrption: 调用RUT工具
    args:
    ...
    for i in module_3:
        ...
        cmd += 'title %s\r\n' % lruget(arg)
        java_cmd1 = 'java -jar rut-2.0-SNAPSHOT.jar %s %s %s ' % (lruget(arg), ip, dirname)
        cmd += java_cmd1
        ...
        if block is False:
            ...
            #stdout = subprocess.PIPE, stderr= subprocess.STDOUT
            sub_p[arg] = subprocess.Popen(java_cmd, shell = False, cwd = os.path.abspath(os.path.dirname(__file__)), creationflags =subprocess.CREATE_NEW_CONSOLE)
            print(java_cmd)
    ...
    if block:
        while True:
            rt_code, fail_list = update_check(sub_p, act)
            if rt_code != code_dict['running']:
                if len(fail_list) > 0:
                    rt_code = code_dict['fail']
                else:
                    rt_code = code_dict['success']
                break
            else:
                time.sleep(2)
        ...
        while rt_code == code_dict['fail'] and retry > 0:
            retry -= 1
            rt_code, fail_list = update(fileType, act, fail_list, dirname, block, retry = -10)
        ...
        if retry >= 0:
            if rt_code != code_dict['success']:
                raise Exception(rt_code, fail_list)
            else:
                return code_dict['success']
        else:
            return rt_code, fail_list
    else:
        return sub_p

def update_cfg(dst, cfgname, retry = 2):
    assert(retry >= 0)
    ...
    if len(dst) == 2:
        ...
        if isIPreachable(slot_ip[dst[0]]):
            key = dst[0]
            cmd += '%s %s ./input/%s 00000001 ' % (lruget(dst[0]), slot_ip[dst[0]], cfgname)
            cmd += '%s 0 0' % (lruget(dst[1]))
            
        elif isIPreachable(slot_ip[dst[1]]):
            key = dst[1]
            cmd += '%s %s ./input/%s 00000001 ' % (lruget(dst[1]), slot_ip[dst[1]], cfgname)
    ...
    sub_p = {}
    sub_p[key] = subprocess.Popen(cmd, shell = False, cwd = os.path.abspath(os.path.dirname(__file__)), creationflags =subprocess.CREATE_NEW_CONSOLE)
    sub_p[key].wait()

    if sub_p[key].returncode != 0:
        update_cfg(dst, cfgname, retry - 1)

def update_dump(module_3, mode = 'AB', dirname = os.path.dirname(__file__) + '/dump'):
    global default_ip
    if not isinstance(module_3, list):
        module_3 = inputsplit(module_3)  
    #module_3 = VCUs_on(module_3)
    ...
    for i in module_3:
        ...
        if 'A' in mode:
            cmd = java_cmd + 'A'
            key = str(arg) + '_A' 
            sub_p[key] = subprocess.Popen(cmd, shell = False, cwd = dirname, creationflags =subprocess.CREATE_NEW_CONSOLE)
        if 'B' in mode:
            cmd = java_cmd + 'B'
            key = str(arg) + '_B'
            sub_p[key] = subprocess.Popen(cmd, shell = False, cwd = dirname, creationflags =subprocess.CREATE_NEW_CONSOLE)
