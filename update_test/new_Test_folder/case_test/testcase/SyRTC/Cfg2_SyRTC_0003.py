# !/usr/bin/python
# _*_ coding: utf-8 _*_
import sys, os
'''添加系统路径'''
#region
sysPath_temp = []
paths = os.path.abspath(os.path.dirname(__file__)).split('\\')
for i in range(-1, -len(paths), -1):
    #path = '\\'.join(paths[:i] + [paths[i]])
    if 'case_test' in paths[i]:
        path = '\\'.join(paths[:i] + [paths[i]])
        break
sysPath_temp.append(path)
sys.path += [path for path in sysPath_temp if path not in sys.path]
import caseFunc
from caseFunc import Log, Config, FrameLib, Conf, Tag, Simulator, sleep, Public, gevent, funclog
import threading
import time
time.sleep = sleep
form = Config.form
#endregion

def LogPath_set(path):
    dir_path = os.path.dirname(__file__) + '/Log/%s/%s' % (os.path.splitext(os.path.basename(__file__))[0], path)
    Log.path_set(dir_path)
def before_main():
    LogPath_set('default')
'''''''''''' # 以上为固定头
requirement = caseFunc.StubRequirement(slots='3') #slots='2,3-8'  # 当前用例的环境需求 ，该变量必须赋值


def step2():
    global mvcu
    LogPath_set('02')
    sims = Simulator('cfg2')
    sims.openwith(1, 'E')
    form.tst_set(mvcu, Tag.tag_send('000301' + '11'))
    form.tst_set('3', Tag.tag_send('000301' + '11'))
    sleep(120)

def step3():
    LogPath_set('03')
    form.tst_set(mvcu, Tag.tag_send('000302' + '11'))
    sleep(30)
    caseFunc.power_restart()
    form.get_eventlog(mvcu)


def step4():
    LogPath_set('04')
    form.tst_set(mvcu, Tag.tag_send('000301' + '11'))
    form.tst_set('3', Tag.tag_send('000301' + '11'))
    sleep(60)

def step5():
    LogPath_set('05')
    form.tst_set('3', Tag.tag_send('000303' + '11'))
    sleep(30)
    caseFunc.power_restart()
    form.get_eventlog('3')

def main():
    global mvcu
    mvcu = form.get_funcMode('2,16')
    mvcu = [ k for k, v in mvcu.items() if v == 'NORMAL']
    step2()
    step3()
    step4()
    step5()

'''使用例可以作为单独的模块执行''' # 格式固定
if __name__ == "__main__":
    '''使用例可以作为单独的模块执行'''
    
    from udp_loglib import TestPlatForm, gevent_join
    form = TestPlatForm(18125, 18125)
    Config.form = form
    
    def confSet():
        FrameLib.respondCheck()  # 会检查之前所有的需回复消息，检查完毕后结束
        t = threading.Thread(target=main)
        t.start()
        t.join()
        Public.normal_end()
        Public.mPrint('exit')
    st = time.time()
    
    gevent_join(confSet)
    print(time.time() - st)

    