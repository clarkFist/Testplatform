...
class Server(MyTcp):
    ...
    def on_cmd(self, data_loaded):
        ...
        self.gevent_dict[key] = gevent.spawn(func, *args)
        return key
    ...
    def build(self, name):
        ...
        self.state_dict.setdefault('build', BoundedSemaphore(1)).wait(180)
    ...
    def copy_image(self, name, dstpath = None):
        ...
        return 0, '%s to %s' % (out_path, dstpath)

    def copy_src(self, name):
        #if self.state_dict.get('build', False) is True:
        #    return 1, 'is building'
        self.state_dict.setdefault('copy_src', BoundedSemaphore(1)).wait(15)
        with self.state_dict.setdefault('copy_src', BoundedSemaphore(1)):
    ...
    def openTagInit(self, name, Tag_list):
        if self.state_dict.get('build', BoundedSemaphore(1)).locked() is True:
            return 1, 'is building'
        elif self.state_dict.get('copy_src', BoundedSemaphore(1)).locked() is True:
            return 1, 'is copy_src'

        self.state_dict.setdefault('openTagInit', BoundedSemaphore(1)).wait(10)
        with self.state_dict.setdefault('openTagInit', BoundedSemaphore(1)):
            ...
            VMConfig.print_func('openTagInit %s success' % (str(Tag_list) ))
            return 0, ''
...
class Client(MyTcp):
    ...
    def send(self, msg):
        ...
        try:
            ...
            raise Exception('no vmtool server in VM')
...
def vm_build_update(slots, open_initTag = None):
    ...
    light_update.update('auto', 0, slots)
