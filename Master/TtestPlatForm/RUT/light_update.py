    ...
    for key, p in sub_p.items():
        if p.poll() is not None:
            ...
            with open(cur_name, 'r') as f:
                log_content = f.read()
            if act == 0 or act == 6:
                pattren = ('Get command status for CPU B.+Success')
                # pattren = ('Restore REPROG safety flag on CPU A.+ Getcommand status for CPU A'+
                #  '.+ Restore REPROG safety flag on CPU B.+Get command status for CPU B.+Success')
            elif act == 1:
