...
def step1():
    LogPath_set('01')
    caseFunc.vm_build_update(svcu, 'VCU_SVCU_SwRTC_1018_1')
    time.sleep(150)
    assert('NORMAL' == form.get_funcMode(svcu)[int(svcu)])


def step2():
    LogPath_set('02')
    caseFunc.vm_build_update(svcu, 'VCU_SVCU_SwRTC_1018_2')
    assert(caseFunc.check_allboard(svcu, timeout=150))


def main():
    global svcu
    # svcu = caseFunc.get_slots_withSerial()[1]['A'][0] # 获取一块板卡
    svcu = '4'
    step1()
    step2()
