...
#include "tst_tsk.h"
#endif
#ifdef TestPlatform //svcu only
  INT32S TB_SEND_NUM =  2;
  INT32S TB_RECV_NUM = 3;
  INT32U Tst_WriteTemp[2];
  MSTEP_T_IodbData TB_readIOdBData[128];
  MSTEP_T_IodbData TB_writeIOdBData[128];
#else
  #define TB_SEND_NUM   1   //32
  #define TB_RECV_NUM   1    //128
...
#ifdef TestPlatform
#define VARY_WRITETEMP
void Tst_F_write(INT32U* WriteTemp)
{
	INT8U mpos = 0;
	INT8U temp = 0;
	for(;mpos<32;mpos++)
	{
		if(mpos < TB_SEND_NUM )
		{
			temp = WriteTemp[0]>>(31-mpos);
			TB_writeIOdBData[mpos].value = temp&0x01;
		}
		else
			break;
	}
...
static void Tst_F_WiriteTempSet( Tst_UDPConfArgs* args )
{
	Tst_WriteTemp[0] = args->val1;
	Tst_WriteTemp[1] = args->val2;
}
static void Tst_F_SR_NUM_TempSet( Tst_UDPConfArgs* args )
{
	INT32U val1 = args->val1;
	INT32U val2 = args->val2;
	if(val1 > 0 && val1 <= 128 && val2 > 0 && val2 <= 128)
	{
			TB_SEND_NUM  = val1;
			TB_RECV_NUM = val2;
	}
	Tst_F_PrintReport("Send:%d,Recv:%d\n",TB_SEND_NUM,TB_RECV_NUM);
}
#endif
/**
******************************************************************************
...
#ifdef TestPlatform
	Tst_F_Creat();
	Tst_F_ConfPonterAdd(13,Tst_F_WiriteTempSet);
	Tst_F_ConfPonterAdd(14,Tst_F_SR_NUM_TempSet);
#endif
    return Status;
}
...
{	
        ...
        || (functionalMode == ALSTOM_C_FUNCTIONAL_GUEST_RESTRICTIVE))
    {
		...
		if(0 != TB_RECV_NUM)
		{
			...
			{
				GUEST_M_FPGA_Write(GUEST_C_OUTPUTLIGHT_LAMP_ADDR, TB_Receive_Data[(g_CycleNO%(TB_index+1))]);
			}		
		}
...
#ifdef VARY_WRITETEMP
	Tst_F_write(Tst_WriteTemp);
#endif
#endif
...
/*20200629, add app interface to get PDO forgive cycle, CR:2245*/
