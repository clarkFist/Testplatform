...
#define GUEST_C_FPGA_OUTPUTLIGHT_LAMP (0x000Au)  /*  */
...
void GUEST_F_WriteMNTDataAndSend(INT16U cmd);
static void GUEST_F_SendMNTData(INT8U * buf, INT32U size);


static INT16U GUEST_F_ReadPanel(void)
{
    INT16U panel = 0u;
    
    GUEST_M_FPGA_Read(GUEST_C_FPGA_OUTPUTLIGHT_LAMP, panel);
...
void GUEST_F_WriteMNTDataAndSend(INT16U cmd)
{
    GUEST_T_MNT MNT_Info;
    INT32U i = 0u;
    INT8U SumPort = 0u;
    INT8U ByteNum =0u;
    INT16U PanelLight = 0u;
    INT32U MessLength =0;
    static INT8U index = 0u;
    static INT8U MNT_Pkgs[GUEST_C_MNT_TFTP_BUF_SIZE];
    static INT8U MNT_SendBuf[GUEST_C_MNT_TFTP_BUF_SIZE];
    
    const GUEST_T_CFG *pCFGAddr = GUEST_F_GetConfigPointer();
    GDF_M_NULL_ASSERT(pCFGAddr);
    
    STD_F_Memset((INT8U*)&MNT_Info,0x0u,sizeof(GUEST_T_MNT));
    STD_F_Memset((INT8U *)MNT_SendBuf, 0u, sizeof(MNT_SendBuf));
	...
	{
		ByteNum = ((SumPort / 8u) + 1u);
	}

	ByteNum = 2u; /*保证监测消息肯定是最大的*/

    /*子包头：1bytes-CAN总线上传输的帧号，1byte-主备标志，1byte-预留字段，1byte-消息长度，
    */
    MNT_Info.SYN_FRAME_NUM = MSTEP_F_SyncCurNumGet();   /* the number of sync frame, VCU have no interface for it, future */       
    if(2u == index)
    {
    MNT_Info.MST_STB_STATUS = 0x01u;  /* 主备无差别，按主板卡处理 bit2:1-有后续子包，0-无后续子包*/    
    }
    else
    {
    MNT_Info.MST_STB_STATUS = 0x05u;  /* 主备无差别，按主板卡处理 bit2:1-有后续子包，0-无后续子包*/    
    }
    MNT_Info.MSG_RESERVE = 0x00u;   /*预留字段   */
    MNT_Info.MSG_LEN = 7u + ByteNum;
    
    /*1bytes-实际用到面板灯个数，1bytes-1个面板灯占用的bit数，
    1bytes-PANEL_LIGHT字段所占字节数，ByteNum-面板输出灯状态，
    1byte-采集码位个数，恒为0
    1byte-使用到的驱动码位个数，1byte-每个码位占用的BIT数，
    1byte-CMD字段所占字节数，ByteNum-输出命令*/
    MNT_Info.PANEL_LIGHT_NUM = 0x10u;
    MNT_Info.PANEL_BIT_NUM = 0x01u;
    PanelLight = GUEST_F_ReadPanel();
    MNT_Info.PANEL_LIGHT = STD_M_SWAP_ENDIANNESS_DATA16(PanelLight);
    MNT_Info.STATUS_VAR_NUM = 0x0u;
    MNT_Info.CMD_VAR_NUM = SumPort;
    MNT_Info.CMD_BIT_NUM = 0x01u;
        
    if(2u == ByteNum)
    {
        MNT_Info.CMD = STD_M_SWAP_ENDIANNESS_DATA16(cmd);
    }
    else if(1u == ByteNum)
    {
        MNT_Info.CMD = STD_M_SWAP_ENDIANNESS_DATA16(cmd);
    ...
    MessLength = 4U + 7U + ByteNum + 27u;   /*保证一包40字节*/
#if 0        
#ifdef ALSTOM_CONFIG_CPU_A
    DTXT_REPORT3(DTXT_REPORT_INFO, "monitor message, only printf 4 byte : ", MNT_Info.MSG_TYPE , MNT_Info.MST_STB_STATUS,*(INT16U*)(&(MNT_Info.MST_STB_STATUS)));
#endif
#endif
    STD_F_Memset((INT8U *)&MNT_Pkgs[MessLength * index], 0u, MessLength);
    STD_F_Memcpy((INT8U *)&MNT_Pkgs[MessLength * index], sizeof(GUEST_T_MNT), (INT8U *)&MNT_Info, MessLength);
    
    index ++;
    index = (index % GUEST_C_MNT_NUM);



    if(0u == index)
    {    
        STD_F_Memcpy((INT8U *)MNT_SendBuf, sizeof(MNT_SendBuf), MNT_Pkgs, MessLength * GUEST_C_MNT_NUM);

        GUEST_F_SendMNTData((INT8U *)&MNT_SendBuf, MessLength * GUEST_C_MNT_NUM);
    }
...
static void GUEST_F_SendMNTData(INT8U * buf, INT32U size)
{
	...
	static ALSTOM_T_Status TFTPClientWriteErrNo = ALSTOM_C_ERROR;
	INT8U  errNO = 0u;
    
	GDF_M_NULL_ASSERT(buf);

#ifdef ALSTOM_CONFIG_CPU_A
	OSW_F_FlagAccept(MSTEP_TFTP_V_pFlagGroup, 1, OSW_C_FLAG_WAIT_SET_ANY + OSW_C_FLAG_CONSUME, &err);

	if(ALSTOM_C_NO_ERROR != err)
	{
		if(OSW_C_FLAG_ERR_NOT_RDY != err)
		{
			GDF_M_FAULT_EXT(0u, err, "");
		}
		else
		{
			DTXT_REPORT1(DTXT_REPORT_WARNING, "GUEST_F_SendMNTData: OSW_F_Flag not ready. err :", err);
		}
	}
	else
	{

		MSTEP_F_TFTPClientWrite(0x7f020000 | 0x79u, "mnt_01", buf, size, MSTEP_TFTP_V_pFlagGroup, 1, &TFTPClientWriteErrNo);        
	} 
#else
#pragma PRQA_MESSAGES_COMMENT "Useless assignement to avoid compilation warning: to be removed if size is used"
    err = err;
    TFTPClientWriteErrNo = TFTPClientWriteErrNo;
    errNO = errNO;
    size = size;
#endif
    return ;
