...
class Mib(SubMoudleBase):
    ...
    def __generation(self, path):
        ...
        sup_dict = self._read_dict['iso']
        for name, sup, value in mib_sub:
            self._name_dict[name] = self._name_dict[sup] + [name]
            name_dict = {}
            sup_dict = self._read_dict_get(sup)
            sup_dict[name] = name_dict
            #self._read_dict_get(sup)[name] = {}

            name_dict['__value'] = sup_dict['__value'] + '.' + value
            
            if 'number' in name.lower() and 'entry' not in sup.lower():
                sup_dict['number'] = name_dict['__value']
            elif 'table' in name.lower():
                name_dict['number'] = sup_dict['number']

            if 'table' in sup.lower():
                name_dict['number'] = sup_dict['number']
                
            if 'entry' in sup.lower():
                name_dict['mode'] = 'entry'
                name_dict['number'] = sup_dict['number']
            else:
                name_dict['mode'] = 'element'
    ...
    def _get(self, objID, snmpip = '', add='.0'):
        ...
        objID += add
        a = self._sem_dict[snmpip].acquire(blocking = True, timeout=30)
        if not a and snmpip not in self._mib_ip:
            return Trys.todoException('cant get snmp')
        else:
            assert a
        try:
            res = snmp.getCmd(snmp.SnmpEngine(), snmp.CommunityData('URBALIS', mpModel=0),
                              snmp.UdpTransportTarget((snmpip, 161), timeout = 1, retries = 0), snmp.ContextData(), snmp.ObjectType(snmp.ObjectIdentity(objID)))
    ...
    def _walk(self, objID, end_idx, snmpip):
        ...
        try:
            for e1, e2, e3, tabble in snmp.nextCmd(snmp.SnmpEngine(), snmp.CommunityData('URBALIS', mpModel=0),
                                                   snmp.UdpTransportTarget((snmpip, 161), timeout= 10), snmp.ContextData(), 
    ...
    def get_value(self, name, lru = 0, outfile = None, raise_error = True):
        ...
        if rt is None: # 第一次获取失败
            self._mib_ip.clear() # 重新对ip 赋值
            for ip in self._default_ip:
                self._mib_ip.append(ip)
            rt = get()
            if rt is None: # 第二次仍旧失败
                if raise_error:
    ...
    def get( enter_dict ):
        for key, value in enter_dict.items():
            if not isinstance(value, dict):
                continue
            if value['mode'] == 'entry' or value['mode'] == 'element':
                print(key, mib.get_value(key))
            else:
                get(value)
    while True:
        start = time.time()
