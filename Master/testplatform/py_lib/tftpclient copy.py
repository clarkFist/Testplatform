...
class TftpClient:
    """
        ...
        --------------------------------------
    """
    def __init__(self, remoteIp, i = -3):
        
        # 创建socket实例
        self.socketClient = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        if i >= 0:
            self.socketClient.bind(('20.2.1.100', 59534 + i))
        self.socketClient.settimeout(3)
        self.remoteIp = remoteIp

    def read_request(self, dstfilename, port = 69, srcfilename= None, flag = False):
        """ TFTP 下载"""
    ...
    def write_request(self, dstfilename, port = 69, srcfilename = None, data = None, flag=False):
        """TFTP 上传, 只发请求， 不write"""
...
if __name__ == "__main__":
    demo1 = TftpClient(remoteIp='20.2.1.11')
    demo2 = TftpClient(remoteIp='20.1.1.11')
    import re
    try:
        demo1.write_request('cop_v001_open', port = 69, data='02', flag=False)
    except:
        funcMode = -1
    else:
        data = demo1.read_request('viv_v001_oHDGB45G6', 1064)
        data = binascii.b2a_hex(data).decode()
        name = 'viv_v001_%s%s%s' % ('i', data, '')
        data = demo1.read_request(name, 1064)
        data = binascii.b2a_hex(data).decode()
        mouduleId, funcMode, fuseState, reprogAutho = re.findall('.{2}', data)
    if int(funcMode) in [0, 3]:
        mvcu = demo1
        mvcu_port = 1064
        mvcu_ip = mvcu.remoteIp
        print('mvcu is 1_2')
    else:
        demo2.write_request('cop_v001_open', port = 69, data='04', flag=False)
        data = demo2.read_request('viv_v001_oHDGB45G6', 1104)
        data = binascii.b2a_hex(data).decode()
        name = 'viv_v001_%s%s%s' % ('i', data, '')
        data = demo2.read_request(name, 1104)
        data = binascii.b2a_hex(data).decode()
        mouduleId, funcMode, fuseState, reprogAutho = re.findall('.{2}', data)
        assert(int(funcMode) in [0, 3])
        mvcu_port = 1104
        mvcu = demo2
        mvcu_ip = mvcu.remoteIp
        print('mvcu is 1_16')
    ...
    # def func(i):
    #     gevent.sleep(0.05 * i)
    #     mvcu = TftpClient(remoteIp=mvcu_ip, i = i)
    #     mvcu.write_request('cop_v001_open', port = 69, data='04', flag=True)
    #     mvcu.read_request(name, mvcu_port, flag = True)
    while True:
        for i in range(0, 30): #连接数
            time.sleep(0.05)
            mvcu = TftpClient(remoteIp=mvcu_ip, i = i)
            #mvcu.write_request('cop_v001_open', port = 69, data='04', flag=True)
            #mvcu.read_request(name, mvcu_port, flag = True)
            mvcu.read_request('viv_v001_oHDGB45G6', mvcu_port, flag=True)
        print('step')
        time.sleep(20)
        try:
            demo1.write_request('cop_v001_open', port = 69, data='02', flag=False)
