...
class TftpClient:
    """
        ...
        --------------------------------------
    """
    def __init__(self, remoteIp = '20.1.1.10', i = 0):
        
        # 创建socket实例
        self.socketClient = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        #self.socketClient.bind(('20.1.1.100', 59534 + i))
...
if __name__ == "__main__":
    ...
    #data_s = demo.read_request('viv_v001_oHDGB45G6', port = 1464)
    import gevent
    g_lib = []
    def func(i):
        demo = demo_list[i]
        gevent.sleep(0.010 * i)
        try:
            demo.write_request('cop_v001_open', port = 69, data='16')
        except:
            pass
    demo_list = []  
    for i in range(1, 910):
        demo_list.append(TftpClient(i = i))

    for i in range(1, 800):
        data = hex(i)[2:].rjust(2, '0')
        g = gevent.spawn(func, i)
        g_lib.append(g)
    gevent.joinall(g_lib)