...
def before_main():
    ...
    LogPath_set('default')
'''''''''''' # 以上为固定头
requirement = caseFunc.StubRequirement(slots=None, svcu_count=1, serial={'SVCUA': 1}) #slots='2,3-8'  # 当前用例的环境需求 ，该变量必须赋值


def main():
    form.Serial_Flag = True   
    global svcus
    # svcus = caseFunc.get_slots_withSerial()[1]['A'][0] # 获取一块板卡
    svcus = '4'
    LogPath_set('02')
    caseFunc.vm_build_update(svcus, 'VCU_SVCU_SwRTC_1064_1')
    time.sleep(150)
    assert('NORMAL' == form.get_funcMode(svcus)[int(svcus)])

    LogPath_set('03')
    caseFunc.vm_build_update(svcus, ['VCU_SVCU_SwRTC_1064_1', 'VCU_SVCU_SwRTC_1064_2'])
    time.sleep(150)
    assert('VIV_ERROR' == form.get_funcMode(svcus)[int(svcus)])
