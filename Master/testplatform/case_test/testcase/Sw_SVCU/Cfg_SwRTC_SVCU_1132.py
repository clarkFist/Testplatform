...
def before_main():
    ...
    LogPath_set('default')
'''''''''''' # 以上为固定头
requirement = caseFunc.StubRequirement(slots=None, svcu_count=1) #slots='2,3-8'  # 当前用例的环境需求 ，该变量必须赋值

def step2():
    LogPath_set('02')
    form.tst_set(dst_slot, Tag.tag_send('11320111')) 
    
    # caseFunc.vm_build_update(dst_slot, 'VCU_SVCU_SwRTC_1132_1')
    # time.sleep(150)
    # assert('NORMAL' == form.get_funcMode(dst_slot)[int(dst_slot)])
    time.sleep(2)

def step3():
    LogPath_set('03')
    
    form.tst_set(dst_slot, Tag.tag_send('11320211')) 
    time.sleep(120)
...
if __name__ == "__main__":
    ...
    def confSet():
        ...
        #assert(caseFunc.check_allboard())
