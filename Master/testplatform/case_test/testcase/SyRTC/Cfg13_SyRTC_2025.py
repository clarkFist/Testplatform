...
def before_main():
    ...
    LogPath_set('default')
'''''''''''' # 以上为固定头
requirement = caseFunc.StubRequirement(slots='2.16.3', svcu_count=None) #slots='2,3-8'  # 当前用例的环境需求 ，该变量必须赋值
assert(__name__ == "__main__") # 只能手动

def step1():
    LogPath_set('01')
    form.update_cfg('2.16', 'CSZ_MVCU1344_RSSP1_28SVCU_6AUTO3_3')
    time.sleep(600)
    caseFunc.check_allboard('2.16', timeout=100)
...
def step3():
    LogPath_set('03')
    form.update_cfg('2.16', 'CSZ_MVCU1344-FSFB2-0722')
    time.sleep(600)
    caseFunc.check_allboard('2.16', timeout=100)
    caseFunc.power_restart()
...
def step4():
    LogPath_set('04')
    caseFunc.check_allboard('3', timeout=100)
    sims = Simulator('cfg1')
    sims.openwith(1, '8')
    time.sleep(40)
