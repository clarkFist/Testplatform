...
BOOLEAN TestFlag_2 = FALSE;
BOOLEAN TestFlag_3 = FALSE;
BOOLEAN TestFlag_4 = FALSE;
BOOLEAN TestFlag_5 = FALSE;
BOOLEAN TestFlag_6 = FALSE;

#define TB_SEND_NUM  2 //32
#define TB_RECV_NUM  3//128
...
{
#ifdef VCU_TB
	if (TRUE == IsTagOpenInit(VCU_SyRTC_0071002))
	{
		return FALSE;
	}
#endif
...
FCT MainLoop
*****************************************************************************/
ALSTOM_T_Status GUEST_F_MainProcess(MSTEPINC_E_FunctionalMode functionalMode)
{
	INT8U i = 0;
	 INT8U TB_index = 0;
    INT8U TB_shift = 0;
    INT32U TB_Receive_Data[4];
    static INT32U g_ProCycleNO = 0;
    TB_Receive_Data[0] = 0;
    TB_Receive_Data[1] = 0;
    TB_Receive_Data[2] = 0;
    TB_Receive_Data[3] = 0;
        ...
        || (functionalMode == ALSTOM_C_FUNCTIONAL_GUEST_RESTRICTIVE))
    {
		...
		#ifdef VCU_TB
		if(0 == g_ProCycleNO%1000)
		{
			Tst_F_Report(4,DTXT_C_LOC,"Receive SDB data",3,TB_readIOdBData[0].value,TB_readIOdBData[1].value,TB_readIOdBData[2].value,0,0,0);   	 	
		}
		#endif	
		GUEST_M_FPGA_Write(GUEST_C_OUTPUTLIGHT_LAMP_ADDR, TB_Receive_Data[0]);		
				
//		GUEST_M_FPGA_Write(GUEST_C_OUTPUTLIGHT_LAMP_ADDR, TB_readIOdBData[g_ProCycleNO%TB_RECV_NUM].value << (g_ProCycleNO%16));
		
//		Tst_F_Report(DTXT_REPORT_INFO, DTXT_C_LOC, "request SDB data:", 4, TB_Receive_Data[0], TB_Receive_Data[1], TB_Receive_Data[2], TB_Receive_Data[3], 0, 0);   
		for (i = 0; i < TB_SEND_NUM; ++i)    //TB_SEND_NUM为提交的数据个数，应参考配置文件，超出最大值系统会宕机
		{  			
			//TB_writeIOdBData[i].value = g_ProCycleNO%2;   //TB_writeIOdBData[i].dataId 要先初始化
			TB_writeIOdBData[i].value = 0;   //TB_writeIOdBData[i].dataId 要先初始化
			TB_writeIOdBData[i].dataId = i+1;
		}
	...
	if(TRUE == IsTagOpen(VCU_SyRTC_0061_1)||TRUE == IsTagOpen(VCU_SyRTC_0001_1))
    {
		TB_writeIOdBData[0].value = 1;
		TB_writeIOdBData[1].value = 1;
		Tst_F_Report(4,DTXT_C_LOC,"recv SDB data",4u,TB_Receive_Data[0],TB_Receive_Data[1],TB_Receive_Data[2],TB_Receive_Data[3],0u,0u);   
	}
    if(TRUE == IsTagOpen(VCU_SyRTC_0006_2)||TRUE == IsTagOpen(VCU_SyRTC_2029_1))
	{
	//	PRINT_F_Report(4,DTXT_C_LOC,"SYS0001",1,IsTagOpen(VCU_SyRTC_0001_1),0,0,0,0,0);  
		TB_writeIOdBData[0].value = 1;
		TB_writeIOdBData[1].value = 1;
		Tst_F_Report(4,DTXT_C_LOC,"SYSRTC OPEN",0,0,0,0,0,0,0);   
	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0060_1)||TRUE == IsTagOpen(VCU_SyRTC_0030_1)||TRUE == IsTagOpen(VCU_SyRTC_0062_1)||TRUE == IsTagOpen(VCU_SyRTC_0064_1)||TRUE == IsTagOpen(VCU_SyRTC_0065_1)||TRUE == IsTagOpen(VCU_SyRTC_2027_1))
	{  
    	TB_writeIOdBData[0].value = 1;
    	TB_writeIOdBData[1].value = 0;
    	TB_writeIOdBData[2].value = 1;
    	Tst_F_Report(4,DTXT_C_LOC,"tag open Send SDB data",4,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,TB_writeIOdBData[2].value,TB_writeIOdBData[3].value,0,0);   		
	}
    if(TRUE == IsTagOpen(VCU_SyRTC_0069_1)||TRUE == IsTagOpen(VCU_SyRTC_0070_1))
	{  
    	TB_writeIOdBData[0].value = 1;
    	TB_writeIOdBData[1].value = 0;
    	TB_writeIOdBData[2].value = 1;
		...
		Tst_F_Report(4,DTXT_C_LOC,"recv SDB data",4u,TB_Receive_Data[0],TB_Receive_Data[1],TB_Receive_Data[2],TB_Receive_Data[3],0u,0u);  
    	//Tst_F_Report(4,DTXT_C_LOC,"tag0069 open Send SDB data",4,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,TB_writeIOdBData[2].value,TB_writeIOdBData[3].value,0,0);   		 
	}
    if(TRUE == IsTagOpen(VCU_SyRTC_0004_2)||TRUE == IsTagOpen(VCU_SyRTC_0068_1)||TRUE == IsTagOpen(VCU_SyRTC_2001_1)||TRUE == IsTagOpen(VCU_SyRTC_2003_1) )
  	{  
        for (i = 0; i < TB_SEND_NUM; ++i)    //TB_SEND_NUM为提交的数据个数，应参考配置文件，超出最大值系统会宕机
		{  			
			TB_writeIOdBData[i].value = (i+1)%2;   //TB_writeIOdBData[i].dataId 要先初始化
			TB_writeIOdBData[i].dataId = i+1;
		}
      	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC," Send SDB data",4,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,TB_writeIOdBData[2].value,TB_writeIOdBData[3].value,0,0);   		 
  	}
    if(TRUE == IsTagOpen(VCU_SyRTC_0005_2))
	{  
    	TB_writeIOdBData[0].value = 0;
    	TB_writeIOdBData[1].value = 1;
    	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"tag0005 open Send SDB data",2,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,0,0,0,0);   		 
	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0067_1)||TRUE == IsTagOpen(VCU_SyRTC_2022_1))
	{  
    	TB_writeIOdBData[0].value = 1;
    	TB_writeIOdBData[1].value = 0;
    	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"tag0067 open Send SDB data",2,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,0,0,0,0);   		 
	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0031_1))
	{  
    	TB_writeIOdBData[0].value = 1;
    	TB_writeIOdBData[1].value = 0;
    	TB_writeIOdBData[2].value = 1;
    	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"tag0031 open Send SDB data",4,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,TB_writeIOdBData[2].value,TB_writeIOdBData[3].value,0,0);   		 
	}
    if(TRUE == IsTagOpen(VCU_SyRTC_0032_1))
	{  
    	TB_writeIOdBData[0].value = 1;
    	TB_writeIOdBData[1].value = 1;
    	TB_writeIOdBData[2].value = 1;
    	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"tag0032 open Send SDB data",3,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,TB_writeIOdBData[2].value,0,0,0);   		 
	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0002_1))
   	{
    	Tst_F_Report(DTXT_REPORT_INFO, DTXT_C_LOC, "request SDB data:", 4, TB_Receive_Data[0], TB_Receive_Data[1], TB_Receive_Data[2], TB_Receive_Data[3], 0, 0);    
   	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_2021_1))
   	{
    	TB_writeIOdBData[0].value = 1;
    		Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"tag2021 open Send SDB data",1,TB_writeIOdBData[0].value,0,0,0,0,0);   
   	}
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0043_1)&&(FALSE == TestFlag_2))
    {
     		Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"VCU_SyRTC_0043_1 open ",1, IsTagOpen(VCU_SyRTC_0043_1),0,0,0,0,0); 
#ifdef VCU_CONF_CPU_A
		STD_F_Memset(TB_logString,0,sizeof(TB_logString));
		STD_F_Snprintf(TB_logString,0x64,"SET_FailureCode 0x60 SVCU");
		MSTEP_F_FailureCodeSet(0x60u,(MSTEPINC_T_FailureContext*)TB_logString);
		DTXT_F_Printf("SET FailureCode--->0x60\r\n");
#endif
		TestFlag_2 = TRUE;
    }
    if(TRUE == IsTagOpen(VCU_SyRTC_0043_2)&&(FALSE == TestFlag_3))
    {
       	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"VCU_SyRTC_0043_2 open ",1, IsTagOpen(VCU_SyRTC_0043_2),0,0,0,0,0); 
#ifdef VCU_CONF_CPU_A
		STD_F_Memset(TB_logString,0,sizeof(TB_logString));
		STD_F_Snprintf(TB_logString,0x64,"FailureCode 0x60 FOR SVCU ");
		MSTEP_F_FailureCodeSet(0x60u,(MSTEPINC_T_FailureContext*)TB_logString);
		DTXT_F_Printf("Context2 SET FailureCode--->0x60\r\n");
#endif
		TestFlag_3 = TRUE;
    }
    
//    PRINT_F_Report(4,DTXT_C_LOC,"TestFlag_3 ",1,TestFlag_3,0,0,0,0,0); 
    
    if(TRUE == IsTagOpen(VCU_SyRTC_0043_3)&&(FALSE == TestFlag_4))
    {
      	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"VCU_SyRTC_0043_3 open ",1, IsTagOpen(VCU_SyRTC_0043_3),0,0,0,0,0); 
#ifdef VCU_CONF_CPU_A
		STD_F_Memset(TB_logString,0,sizeof(TB_logString));
		STD_F_Snprintf(TB_logString,0x64,"FailureCode Set 0x50 SVCU");
		MSTEP_F_FailureCodeSet(0x50u,(MSTEPINC_T_FailureContext*)TB_logString);
			Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"SET FailureCode--->0x50\r\n",0,0,0,0,0,0,0);
#endif
		TestFlag_4 = TRUE;
    }
    if(TRUE == IsTagOpen(VCU_SyRTC_0043_4)&&(FALSE == TestFlag_5))
    {
       	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"VCU_SyRTC_0043_4 open ",1, IsTagOpen(VCU_SyRTC_0043_4),0,0,0,0,0); 
#ifdef VCU_CONF_CPU_A
		MSTEP_F_FailureCodeClear(0x50u);
		DTXT_F_Printf("CLEAR FailureCode--->0x50\r\n");
#endif
		TestFlag_5 = TRUE;
    }    
    if(TRUE == IsTagOpen(VCU_SyRTC_0043_5)&&(FALSE == TestFlag_6))
    {
     		Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"VCU_SyRTC_0043_5 open ",1, IsTagOpen(VCU_SyRTC_0043_5),0,0,0,0,0); 
#ifdef VCU_CONF_CPU_A
     	MSTEP_F_FailureCodeClear(0x60u);
			Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"CLEAR FailureCode--->0x60\r\n",0,0,0,0,0,0,0);
#endif
		TestFlag_6 = TRUE;
    }    		 
//    if(TRUE == IsTagOpen(VCU_SyRTC_0043_6)&&(FALSE == TestFlag_2))
//    {
//    	PRINT_F_Report(4,DTXT_C_LOC,"VCU_SyRTC_0043_6 open ",1, IsTagOpen(VCU_SyRTC_0043_6),0,0,0,0,0); 
//#ifdef VCU_CONF_CPU_A
//    	FCODE_F_FailureCodeClear(15u);
//    	PRINT_F_Printf("CLEAR FailureCode--->15\r\n");
//#endif
//    	TestFlag_2 = TRUE;
//    }  

    if(TRUE == IsTagOpen(VCU_MVCU_SwRTC_3227_2))
    {
    	TB_writeIOdBData[0].value = 1;   //TB_writeIOdBData[i].dataId 要先初始化
    	MSTEP_F_IodbSubmit(1,TB_writeIOdBData);
    		Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"*****TB_writeIOdBData[0].value ***", 1,TB_writeIOdBData[0].value ,0,0, 0, 0, 0);		
    }
    if(TRUE == IsTagOpen(VCU_MVCU_SwRTC_3227_3))
    {     
    	TB_writeIOdBData[0].value = 0;   //TB_writeIOdBData[i].dataId 要先初始化
    	MSTEP_F_IodbSubmit(1,TB_writeIOdBData);
    	Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"*****TB_writeIOdBData[0].value ***", 1,TB_writeIOdBData[0].value ,0,0, 0, 0, 0);		
    }
#endif
...
#ifdef VCU_TB
	if(g_ProCycleNO%50==0)
		Tst_F_Report(DTXT_REPORT_INFO,DTXT_C_LOC,"Send SDB data",2,TB_writeIOdBData[0].value,TB_writeIOdBData[1].value,0,0,0,0);   		
#endif	
    }
	return 0;
}
...
{	
	...
	GUEST_F_MainProcess(functionalMode);	
