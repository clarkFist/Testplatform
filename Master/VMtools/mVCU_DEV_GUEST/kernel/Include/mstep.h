...
/*20190718, add new API functions and update some API function, CR:1633*/
#define  GID_XOR_VALUE_CPUA   0x319B414Cu

#define  GID_XOR_VALUE_CPUB   0x6AC83327u
...
/*20200422, Provide network status (physical layer / communication layer / security layer), CR:2081*/
/*20210803, Provide unsafe network status(Maintenance network/Internal network), CR:2921*/
typedef struct
{
	INT8U Net0_physicalLayerStatus_A;   /*网卡0网络物理层状态-CPUA(维护网),1-连接,2-断开*/
	INT8U Net0_physicalLayerStatus_B;   /*网卡0网络物理层状态-CPUB(内网),1-连接,2-断开*/
	INT8U Net1_physicalLayerStatus_A;   /*网卡1网络物理层状态-CPUA(安全红网),1-连接,2-断开*/
    INT8U Net1_physicalLayerStatus_B;   /*网卡1网络物理层状态-CPUB(安全蓝网),1-连接,2-断开*/
    INT8U communicateLayerStatus_N[6];  /*网络通信层状态--CPUA(红网),0-未配置该会话,1-连接,2-断开*/
    INT8U communicateLayerStatus_R[6];  /*网络通信层状态-CPUB(蓝网),0-未配置该会话,1-连接,2-断开*/
    INT8U safetyLayerStatus_A[6];       /*网络安全层状态-CPUA,0-未配置该会话,1-连接,2-断开*/
    INT8U safetyLayerStatus_B[6];       /*网络安全层状态-CPUB,0-未配置该会话,1-连接,2-断开*/
}MSTEP_T_NetStatusInfo;
 ...
 *
 *****************************************************************************/
/*20200629, add app interface to provide CPU utilization, CR:2240*/
...
/*20210803, delete APIs realted to read parameters , CR:2921*/
...
INT32U MSTEP_F_GidGet(void);

ALSTOM_T_Status MSTEP_F_GidKeyRead(INT32U readAddr, INT8U* pReadBuffer, INT32U readsize);

INT8U MSTEP_F_SyncCurNumGet(void);

BOOLEAN MSTEP_F_Rssp1CycleGet(INT8U EqId, INT32U* pLocalEqTxCycle, INT32U* pRemoteEqTxCycle);

void MSTEP_F_SnmpIpGet(INT32U* pNormalIp, INT32U* pRedundantIp);

INT32U MSTEP_F_SntpIpGet(void);

void MSTEP_F_UnSafetyPortGet(INT32U* pUnSafetyNormPort, INT32U* pUnSafetyRedundPort);

/**
 ******************************************************************************
 * \par Description :
 * Read FuncMode from another MVCU board.\n
 *
 * \par Fault cases:
 *      - pFuncMode is NULL pointer
 *
 * \param pFuncMode  - out  range: [!NULL] \n
 *        Pointer to the readed data
 *
 * \return BOOLEAN:
 *         - FALSE if another MVCU board is not present
 *         - TRUE else.
 *
 * \note The meaning of pFuncMode reference to ALSTOM_E_FunctionalMode.
 *****************************************************************************/
BOOLEAN MSTEP_F_GetOtherMVCUFuncMode(ALSTOM_E_FunctionalMode* const pFuncMode);

/*20200422, Add the function of mode switch by application request, CR:2080*/
/**
 ******************************************************************************
 * \par Description :
 * Request Switch TO Standby Mode. \n
 * \param SwitchId  - in  range: [0x100, 0xFFFFFFFFu] \n
 *        the reason id of request switch to standby by app
 * \note The Supported response frequency of the request is running in Normal mode more than 30s.
 20210803, modify switch standby API input param to record switch reason, CR:2921
 *****************************************************************************/
void MSTEP_F_SwitchStandbyModeRequest(INT32U SwitchId);

/**
 ******************************************************************************
 * \par Description :
 * Read from IODB data.\n
 * \note Data access is done through the sessionId as configured in Key.
 *
 * \par Limitation:
 *      see FeatureIODB for details
 *
 ...
 *      - pValues is NULL pointer
 *
 * \param sessionId  - in  - range: configured in Key [0,5]
 *        the index of session to read into the database
 * \param pBuffer - out - range: [!NULL] \n
 *        Pointer on the readed data
 *\param pLength - out - range: [!NULL] \n
 *        Pointer to the length of readed data
 *
 * \return ALSTOM_T_Status:
 *         - ALSTOM_C_ERROR if something fails
 *         - ALSTOM_C_TIMEOUT if no data received within the set time
 *         - ALSTOM_C_NO_ERROR else.
 *
 * \note The max size of data to read into the database is limited to 978 bytes.

 * \see MSTEP_F_IodbSubmitBySessionId()
 *****************************************************************************/
/*20200228, Add application interface to read and write SDB data through sessionID, CR:1998*/
ALSTOM_T_Status MSTEP_F_IodbRequestBySessionId(const INT8U sessionId, INT8U* const pBuffer, INT32U* const pLength);
 ...
 *
 * \param sessionId - in - range: configured in Key [0,5]
 *        the index of session to write into the database
 * \param pBuffer  - in - range: [!NULL]       \n
 *        Pointer to the data to be written in the database
 *  \param length - in - range: [0,978] bytes
 * \return None
 *
 * \note The max size of data to write into the database is limited to 978 bytes.
 *
 * \see MSTEP_F_IodbRequestBySessionId()
 *****************************************************************************/
/*20200228, Add application interface to read and write SDB data through sessionID, CR:1998*/
void MSTEP_F_IodbSubmitBySessionId(const INT8U sessionId, const INT8U* const pBuffer, const INT32U length);

/*20200422, Provide network status (physical layer / communication layer / security layer), CR:2081*/
/**
 ******************************************************************************
 * \par Description :
 * Read net status from local board.\n
 *
 * \par Fault cases:
 *      - pNetStatus is NULL pointer
 *
 * \param pNetStatus  - out  range: [!NULL] \n
 *        Pointer to the readed data
 *
 * \return ALSTOM_T_Status:
 *         - ALSTOM_C_ERROR if something fails
 *         - ALSTOM_C_NO_ERROR else.
 *
 * \note The The meaning of pNetStatus reference to MSTEP_T_NetStatusInfo.
 *****************************************************************************/
ALSTOM_T_Status MSTEP_F_GetLocalMVCUNetStatus(MSTEP_T_NetStatusInfo* const pNetStatus);

/**
 ******************************************************************************
 * \par Description :
 * Read net status from another board.\n
 *
 * \par Fault cases:
 *      - pNetStatus is NULL pointer
 *
 ...
 *         - ALSTOM_C_NO_ERROR else.
 *
 * \note The meaning of pNetStatus reference to MSTEP_T_NetStatusInfo.
 *****************************************************************************/
ALSTOM_T_Status MSTEP_F_GetAnotherMVCUNetStatus(MSTEP_T_NetStatusInfo* const pNetStatus);

/*****************************************************************************/
/**
* \par Description:  Get the status of external device by sessionId.
*
*
* \param sessionId - - in - range: configured in Key [0,5]
*          the index of session to get net status
* \param pMachineAorB - out  range: [!NULL] \n
*          pointer to the A/B device 0:machineA;1:machineB;3:unknown
* \param pMasterOrSlave - out  range: [!NULL] \n
*          pointer to the master/slaver  0:master;1:slaver;3:unknown
******************************************************************************/
void MSTEP_F_GetExternalDeviceStatus(const INT8U sessionId, INT8U* const pMachineAorB, INT8U* const pMasterOrSlave);

/*****************************************************************************/
/**
* \par Description:  Get the present status of board.
*
*
* \param rackNumber - - in - range:  [1,3]
*        
* \param slotNumber - in  range: [0,15] 
* 
* \return BOOLEAN:
*          -TRUE heartbeat of board can be received(board exist)
*          -FALSE heartbeat of board can not be received(board  not exist)
20210803, add API to get Board Present Status , CR:2921
******************************************************************************/
BOOLEAN MSTEP_F_GetBoardPresentStatus(INT8U const rackNumber,INT8U const slotNumber);

/*****************************************************************************/
/**
* \par Description:  Get system checkword.
*
*
* \param none
* 
* \return INT32U:system checkword.
20210803, add API to get system checkword, CR:2921
******************************************************************************/
INT32U MSTEP_F_SysChkWdGet(void);
