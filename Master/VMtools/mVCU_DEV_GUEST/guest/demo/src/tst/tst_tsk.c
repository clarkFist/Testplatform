...
void Tst_F_FrameProcess()
{
	...
	status = Tst_F_FrameGet(&Tst_FrameBufRecv,&Frame);
		...
		MSTEP_F_K2oo2CpuSend(&status, 1, 0x1f4);
	#else
		MSTEP_F_K2oo2CpuWait(&status, 1, 0x1f4);
	#endif
	if(status == ALSTOM_C_NO_ERROR) //处理接收消息
	{
		#ifdef ALSTOM_CONFIG_CPU_A
			MSTEP_F_K2oo2CpuSend(&TstServerIp, 4, 0x1f4);
		#else
			MSTEP_F_K2oo2CpuWait(&TstServerIp, 4, 0x1f4);
			if( ! Tst_F_isCanAddr(TstServerIp) )
			{
				TstServerIp = Tst_F_canAddrGet(localNodeID_A);
			}
		#endif
		MSTEP_F_K2oo2CpuExchange(&Frame, &Frame,Tst_F_UDPLenGet(10), 0x1f4); //最大VCU收到的消息最大为10
...
void Tst_F_Start()
{
    ...
    Tst_V_pUdpSocket = Tst_F_UDPOpen(Tst_UDP_PORT, Tst_V_TaskInfo, 1u);
	...
	Tst_F_UDPFlush((const MSTEP_T_SocketID *)Tst_V_pUdpSocket);
...
ALSTOM_T_Status Tst_F_Task(ALSTOM_E_FunctionalMode functionalMode)
{
	...
	if(ALSTOM_C_FUNCTIONAL_BOOT_FAILED_RESTRICTIVE > functionalMode)// 除了宕机和初始化失败和更新模式均认为tst的正常模式
	{
		if(Tst_C_FUNCTIONAL_BOOT == Tst_mode) 
		{
			;
		} 
		else
	...
	else
	{
		...
		{
		//	Tst_mode = Tst_C_FUNCTIONAL_CLOSE;//err
			Tst_F_Report(DTXT_REPORT_WARNING, DTXT_C_LOC , "tst_task", 
						1u,functionalMode, 0u,0u,0u,0u,0u);
		}
	}
