...
{	
	...
	status = MSTEP_F_UDPSend(pSocket, destAddr, destPort, (const INT8U * )pData, pData->dataLen);
...
{	
	...
	for(;0<rejectSize;)
	{
		if(tst_cycle >= rejectFlag[rejectStart] )
		{
	...
	if(isStore == TRUE)
	{
		...
		else
		{
			...
			if(preCycle != tst_cycle) //一个tst周期内只会打印一次
			{
		...
		if(Tst_Cycle_UDPReport > 0)//Tst_Cycle_UDPReport=1 时每个tst周期只会打印一次，为0 时不做限制
		{
			...
			rejectFlag[(rejectStart + rejectSize)%rejectMaxSize] = tst_cycle + Tst_Cycle_UDPReport;  //reject 过滤时不会对消息类型（发往串口或udp）做检查
			rejectSize++;
		}
	}
	else
...
void Tst_F_EntrySafetyMode()
{
	...
	MSTEP_F_K2oo2CpuExchange(&test, &test, 1, 0x1f4);
...
void Tst_F_SendMaintMsg(INT8U msgType, const char* fmt,...)
{
	...
	data.dataLen = Tst_UDPDataMaxLen; // 消息总长度
	data.dataLen = (INT16U)Tst_F_UDPLenGet(data.dataLen); //带上头文件的长度
	Tst_F_FrameStore(&Tst_FrameBufSend, 0u,&data);
}
