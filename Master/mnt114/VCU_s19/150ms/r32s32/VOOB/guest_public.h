...
#define GUEST_C_TASK_PERIOD                        3/*5*/                 /**<Guest task period = 5*50ms */
...
/** 监测消息一次发送的条数 */
#define GUEST_C_MNT_NUM                            3u
#define GUEST_C_MNT_TFTP_BUF_SIZE                      (GUEST_C_MNT_NUM * sizeof(GUEST_T_MNT))               /* 存放TFTP消息的内存大小 */
extern OSW_T_pFLAG_GRP* MSTEP_TFTP_V_pFlagGroup;
...
extern INT16U g_ReadIOdBData;
extern void GUEST_F_WriteMNTDataAndSend(INT16U cmd);
...
typedef struct
{
    INT8U SYN_FRAME_NUM;/*Synchronized frame number*/
    INT8U MST_STB_STATUS;/*The Master or Standby status*/
    INT8U MSG_RESERVE;/*The massage reserve*/
    INT8U MSG_LEN;/*The length of the later info*/
    INT8U PANEL_LIGHT_NUM;/*The number of PANEL LIGHT used by the board*/
    INT8U PANEL_BIT_NUM;/*The relation ship of PANEL LIGHT and bit*/
    INT16U PANEL_LIGHT;/*PANEL LIGHT status*/
    INT8U STATUS_VAR_NUM;/*The acquisition status variable number on the board*/
    INT8U CMD_VAR_NUM;/*The driving port number used*/
    INT8U CMD_BIT_NUM;/*The bit number for every variable*/
	INT16U CMD;/*The drive command variables*/
	INT8U  resver[27]; /*保证一包40byte*/ 
} GUEST_T_MNT;
#pragma pack()
