...
#include "tst.h"
#include "dtxt.h"
#define TB_SEND_NUM   128   //32
#define TB_RECV_NUM   32    //128
 #ifdef VCU_TB
	INT8U boardType[2] = {0};
...
//            g_StartTest = TFTP_MSG_PDDM;
	...
	{  			
    	...
    	for (i = 0; i < TB_SEND_NUM; ++i)    //TB_SEND_NUM为提交的数据个数，应参考配置文件，超出最大值系统会宕机
	{  			
		TB_writeIOdBData[i].value = boardType[1]%2;   //TB_writeIOdBData[i].dataId 要先初始化
	}
	if(IsTagOpenInit(VCU_SyRTC_2001_1) &&boardType[0] == 3 && boardType[1] == 2  )
	{
		for (i = 0; i < TB_SEND_NUM; ++i)    //TB_SEND_NUM为提交的数据个数，应参考配置文件，超出最大值系统会宕机
		{  			
