do
    local p_Name = Proto("FSFB2","FSFB2")
	local p_Name2 = Proto("RMS","RMS")
	local p_Name2 = Proto("GAPP","GAPP")
	local p_Name2 = Proto("BSD","BSD")
    --协议的各个字段
	...
	sortdict(my_fields,{"FSFB2",6},{{"App_categort",1},{"MsgType",1},{"SrcAddr",2},{"DstAddr",2}})
	local sub_fields = {}
	local key1 = {"BSD",0}
	local key2 = {"SSE",12}
	local key3 = {"SSR",17}
	...
	sub_fields[key2] = { {"TimeCounter",4},{"SEQENQ_1",4},{"SEQENQ_2",4} }
	sub_fields[key3] = { {"Time_replier",4},{"Time_enquirer",4},{"SEQENQ_1",4},{"SEQENQ_2",4},{"NUM_DATAVER",1}}
	---------------------------------
	local fields_dict ={}
    ...
    local function My_dissector(buf,pkt,root)
		...
		local jud
        if buf_len < 36 then return false end
		...
		for key1,val1 in sortdict_pairs(my_fields) do
			...
			for key2,val2 in pairs(val1) do
				...
				if key1[1] == "FSFB2" and val2[1] == "MsgType" then
    ...
    udp_encap_table:add(40001, p_Name)
    udp_encap_table:add(40000, p_Name)
end
